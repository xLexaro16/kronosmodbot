...
let prefix = "$"
 try { 
   ...
   if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply("\```yml\nMODERATION: You need MANAGE_MESSAGES permission!\n\```");
  if(!message.guild.me.hasPermission("MANAGE_MESSAGES")) return message.reply("\```yml\nMODERATION: I do not have enough permissions!\n\```");
...
if(isNaN(amount) || !Number.isInteger(amount)) return message.channel.send("\```yml\nMODERATION: Please specify a number to clear the messages!\n\```");
if(!amount || amount < 2 || amount > 100) return message.channel.send("\```yml\nMODERATION: Please enter a number between 2 and 100!\n\```")
if(!args[1]) {
